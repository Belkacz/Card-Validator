function checkProvadier(
  ...
  function cardValidtor(cardnumber) {
    if(typeof(cardnumber) === "undefined"){
      cardnumber = prompt('Brak numeru karty, proszę podaj nr. karty');
    ...
    const provaider = checkProvadier(
      cardnumber,
      cardsRequirementsObject,
      cardsRequirementsArray
    );
    if (provaider !== "unknown") {
      const isvalid = luhan(cardnumber);
      if (isvalid == true) {
        console.log(`Karta nr. ${cardnumber} Wystawca karty ${provaider}, numer karty jest poprawny.`);
      } else {
        console.log(`Wystawca karty nr. ${provaider}, ale numer karty jest niepoprawny.`);
      }
    } else {
  ...
  console.log("Walidator kart sprwadza porpawność kart, w momencie odaplenia sprawdza nr. przekazany do funkcji w formie:")
